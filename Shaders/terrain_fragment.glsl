...
vec3 fbm(vec2 pos, out vec2 normal_at_prev_octave) {
    ...
    for(int i = 0; i < int(_Octaves); ++i) {
        ...
        if (i == _ColorOctaveOffset - 1)
            normal_at_prev_octave = grad;
...
void main() {
    ...
    vec2 normal_at_prev_octave = vec2(0.0);
    ...
    vec3 n = _TerrainHeight * fbm(noise_pos.xz, normal_at_prev_octave);
    normal_at_prev_octave *= _TerrainHeight;
    ...
    vec3 slope_normal_at_prev_octave = normalize(vec3(-normal_at_prev_octave.x, 1, -normal_at_prev_octave.y) * vec3(_SlopeDamping, 1, _SlopeDamping));
    ...
    float material_blend_factor = smoothstep(_SlopeRange.x, _SlopeRange.y, 1 - slope_normal_at_prev_octave.y);
